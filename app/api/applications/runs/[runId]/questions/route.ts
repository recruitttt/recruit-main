...
export async function GET(
  ...
  if (run.remoteRunId && run.source === "recruit2-api") {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/apply-lab/runs/${encodeURIComponent(run.remoteRunId)}/questions`, {
        cache: "no-store",
      });
      const body = await response.json().catch(() => null) as { groups?: unknown[]; error?: string } | null;
      if (!response.ok) {
        return Response.json({ ok: false, reason: body?.error ?? `apply_engine_questions_${response.status}` }, { status: response.status });
      }
      return Response.json({
        ok: true,
        groups: Array.isArray(body?.groups) ? body.groups.map((group) => mapRemoteQuestionGroup(group, run)) : [],
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      return Response.json({ ok: false, reason: `apply_engine_questions_unreachable: ${message}` }, { status: 503 });
    }
  }

  return Response.json({ ok: true, groups: getApplyRunStore().getQuestionGroups(runId) });
}
