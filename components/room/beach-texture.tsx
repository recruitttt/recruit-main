...
/**
 * Wide panoramic beach texture. One continuous daytime scene stretched across
 * the entire back wall's window area. Cached once per runtime.
 */
const W = 2048;
const H = 384;
...
function buildBeachTexture(): THREE.CanvasTexture {
  ...
  // Soft sun — high, bright but subtle
  const sunCX = W * 0.74;
  ...
  // Clouds · puffy cumulus spread across the panorama
  const clouds = [
  ...
  // Distant low land strip (subtle) — adds depth to the horizon
  ...
  // Wave highlights across the whole panorama
  ...
  // Sun glitter on water (shimmer column under the sun)
  ...
  // Wet sand strip where ocean meets shore
  ...
  // Foam line — wavy edge along the shore
  ctx.fillStyle = "rgba(255, 255, 255, 0.82)";
  drawFoam(ctx, sandY);

  // Sand texture — a scattering of tonal arcs
  for (let i = 0; i < 18; i++) {
  ...
  // A couple of small seabirds in the sky (tiny M-shapes)
...
function drawPalm(ctx: CanvasRenderingContext2D, baseX: number, baseY: number, height: number) {
  ...
  ctx.bezierCurveTo(
    baseX + 14, baseY - height * 0.35,
    baseX - 10, baseY - height * 0.7,
    baseX + 6, baseY - height
  );
  ctx.stroke();
  // Trunk rings
