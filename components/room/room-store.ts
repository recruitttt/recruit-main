...
// Snapshot the parts of the profile that are useful as model context.
...
async function fetchNextScoutMessage(
...
export const useRoomStore = create<RoomState>((set, get) => ({
  ...
  setIntakePhase: (phase) => {
    ...
    // When questioning starts, fetch Scout's opening line.
    if (phase === "questioning" && get().intakeMessages.length === 0) {
      void (async () => {
        ...
        if (r.ok && r.message) {
          set({
            intakeMessages: [{ role: "assistant", content: r.message }],
            intakePending: false,
          });
        } else {
          // Fall back to a hardcoded opener so the experience never stalls.
          set({
  ...
  submitIntakeAnswer: async (value) => {
    ...
    if (!r.ok) {
      set({
        ...
        intakeMessages: [
          ...
          {
            role: "assistant",
            content: "Sorry, my brain glitched. Try that again?",
          },
        ],
      });
      return;
    ...
    if (r.updates) {
      try {
        mergeProfile(r.updates, "chat", "Scout intake");
      } catch {}
    }
  ...
  finishIntake: () =>
    set({ intakePhase: "inactive", intakePending: false }),
}));

// Helpers for callers that need to know if the in-room intake is complete.
export function hasCompletedRoomIntake(): boolean {
  if (typeof window === "undefined") return false;
  try {
    const flag = window.localStorage.getItem("recruit:intake-done");
    return flag === "1";
  } catch {
    return false;
  }
