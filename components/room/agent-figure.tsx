  ...
  return (
    ...
    >
      <group ref={refs.legL} position={[-0.11, 0.22, 0]}>
        <RoundedBox args={[0.15, 0.22, 0.2]} radius={0.038} smoothness={4} position={[0, -0.11, 0]}>
      ...
      <group ref={refs.body} position={[0, 0.47, 0]}>
        <RoundedBox args={[0.44, 0.52, 0.36]} radius={0.07} smoothness={5}>
          <meshStandardMaterial color={palette.body} roughness={0.6} metalness={0.05} envMapIntensity={0.45} />
          {outlineColor && outlineOpacity > 0 ? (
            <Outlines thickness={0.012} color={outlineColor} opacity={outlineOpacity} transparent />
          ) : null}
        </RoundedBox>
        ...
        <group ref={refs.armL} position={[-0.25, 0.2, 0]}>
          <RoundedBox args={[0.13, 0.4, 0.15]} radius={0.04} smoothness={4} position={[0, -0.2, 0]}>
            <meshStandardMaterial color={palette.body} roughness={0.65} metalness={0.04} />
          </RoundedBox>
          <group ref={refs.wristL} position={[0, -0.4, 0]}>
            <RoundedBox args={[0.12, 0.12, 0.13]} radius={0.036} smoothness={4} position={[0, -0.06, 0]}>
...
function Eye({ eyeRef, x }: { eyeRef: RefObject<THREE.Mesh | null>; x: number }) {
  return (
    <mesh ref={eyeRef} position={[x, 0, 0]} renderOrder={4}>
      <planeGeometry args={[0.088, 0.096]} />
      <meshBasicMaterial color="#050505" toneMapped={false} depthWrite={false} polygonOffset polygonOffsetFactor={-4} polygonOffsetUnits={-4} />
    </mesh>
  );
}
...
function paletteFor(hex: string) {
  ...
  base.getHSL(hsl);
  const body = new THREE.Color().setHSL(hsl.h, Math.min(0.78, hsl.s * 0.9), Math.min(0.62, hsl.l + 0.14));
  const head = new THREE.Color().setHSL(hsl.h, Math.min(0.72, hsl.s * 0.82), Math.min(0.7, hsl.l + 0.22));
  const leg = new THREE.Color().setHSL(hsl.h, Math.min(0.68, hsl.s * 0.8), Math.max(0.22, hsl.l - 0.1));
  const hand = new THREE.Color().setHSL(hsl.h, 0.22, 0.86);
  const patch = new THREE.Color().setHSL(hsl.h, Math.min(0.9, hsl.s * 1.05), Math.min(0.82, hsl.l + 0.32));
