...
export function RoomIntroScout({ phase }: { phase: RoomIntroPhase }) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (phase === "land") {
      ...
      group.scale.set(1 + Math.sin(e * Math.PI) * 0.035, 1 - Math.sin(e * Math.PI) * 0.05, 1 + Math.sin(e * Math.PI) * 0.035);
...
export function IntroRevealGroup({
  ...
  useFrame(() => {
    ...
    if (lastOpacity.current !== null && Math.abs(lastOpacity.current - opacity) < 0.006) return;
...
function collectMaterials(root: THREE.Object3D): CachedMaterial[] {
  ...
  root.traverse((obj) => {
    ...
    for (const mat of materials) {
      ...
      collected.push({ material: m, baseOpacity: m.opacity, baseTransparent: m.transparent, baseDepthWrite: m.depthWrite });
    }
  });
  return collected;
}
...
function updateShadow(shadow: THREE.Mesh | null, position: THREE.Vector3, closeness: number, maxOpacity: number) {
  if (!shadow) return;
...
function heroCameraFor(home: THREE.Vector3, outPos: THREE.Vector3, outLook: THREE.Vector3) {
  outPos.set(home.x, 2.45, home.z + 6.4);
  outLook.set(home.x, 1.05, home.z + 0.15);
}

function outroCameraFor(home: THREE.Vector3, width: number, outPos: THREE.Vector3, outLook: THREE.Vector3) {
  if (width < 640) {
