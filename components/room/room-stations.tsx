...
/* Station 1 · Job board — cork-style bulletin board with pinned papers */
function JobBoard() {
    ...
    <group position={[x, 0, z]}>
      ...
      {papers.map((p, i) => (
        ...
        >
          ...
          {/* Thin line accents suggesting text */}
          <mesh position={[0, 0.14, 0.006]}>
            <planeGeometry args={[0.28, 0.015]} />
            <meshBasicMaterial color="#5C4A30" />
          </mesh>
...
/* Station 2 · Workbench — desk with tilted monitor and papers */
function Workbench() {
    ...
    <group position={[x, 0, z]}>
      ...
      <group position={[0, 0.86, -0.25]} rotation={[-0.18, 0, 0]}>
        ...
        <mesh position={[0, 0.42, 0.032]}>
          ...
          <meshBasicMaterial color="#0F0E0C" />
        </mesh>
        {/* Active screen */}
        <mesh ref={screenRef} position={[0, 0.42, 0.033]}>
          <planeGeometry args={[1.08, 0.58]} />
          <meshBasicMaterial color="#C38A3D" toneMapped={false} />
        </mesh>
        {/* Stand */}
        <mesh position={[0, 0.08, 0]} castShadow>
          <cylinderGeometry args={[0.05, 0.08, 0.12, 16]} />
          <meshStandardMaterial color="#3A332B" roughness={0.55} metalness={0.3} />
        </mesh>
      </group>
      ...
      <group position={[-0.65, 0.89, 0.28]}>
        <mesh castShadow>
          <cylinderGeometry args={[0.07, 0.06, 0.14, 24]} />
          <meshStandardMaterial color="#EEE4CF" roughness={0.85} />
        </mesh>
        <mesh position={[0, 0.08, 0]}>
          <cylinderGeometry args={[0.065, 0.065, 0.005, 24]} />
          <meshStandardMaterial color="#5B3E1E" roughness={0.7} />
        </mesh>
      </group>
...
/* Station 3 · Review panel — three floating persona cards in an arc */
function ReviewPanel() {
  ...
  const cards = [
    { x: -0.85, rot: 0.35, tint: "#F4E4E8" }, // Hiring Manager · warm
    { x: 0, rot: 0, tint: "#F1EFF6" }, // Senior Engineer · cool
    { x: 0.85, rot: -0.35, tint: "#EFF4F1" }, // Recruiter · neutral
  ];
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      <mesh position={[0, 0.04, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#D9C9A7" roughness={0.88} />
      </mesh>
      <mesh position={[0, 0.1, 0]}>
        <cylinderGeometry args={[0.55, 0.55, 0.04, 32]} />
        <meshStandardMaterial color="#CBB990" roughness={0.85} />
      </mesh>
      ...
      <group ref={ref}>
        {cards.map((c, i) => (
          <group
            ...
            rotation={[0, c.rot, 0]}
          >
            <RoundedBox args={[0.72, 0.46, 0.03]} radius={0.04} smoothness={4} castShadow>
              <meshStandardMaterial color={c.tint} roughness={0.7} />
            </RoundedBox>
            {/* Card accent stripe */}
            <mesh position={[0, 0.18, 0.018]}>
              <planeGeometry args={[0.64, 0.03]} />
              <meshBasicMaterial color={station.accent} opacity={0.55} transparent />
            </mesh>
            {/* Lines suggesting review text */}
            {[0.06, 0, -0.06, -0.12].map((y, j) => (
              <mesh key={j} position={[0, y, 0.018]}>
                <planeGeometry args={[j === 0 ? 0.5 : 0.56, 0.018]} />
                <meshBasicMaterial color="#A49880" />
              </mesh>
            ))}
          </group>
        ))}
...
/* Station 4 · Submit terminal — mail-slot kiosk with a soft glow */
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      <mesh position={[0, 1.74, 0]}>
        <boxGeometry args={[0.7, 0.012, 0.08]} />
        <meshBasicMaterial color="#05060A" />
      </mesh>
      {/* Status panel */}
      <mesh position={[0, 1.15, 0.36]}>
        <planeGeometry args={[0.72, 0.38]} />
        <meshStandardMaterial color="#101218" roughness={0.5} emissive="#0B5563" emissiveIntensity={0.6} toneMapped={false} />
      </mesh>
      {/* Status line */}
      <mesh position={[0, 1.17, 0.363]}>
        <planeGeometry args={[0.56, 0.012]} />
        <meshBasicMaterial color={station.accent} toneMapped={false} />
      </mesh>
      <mesh position={[0, 1.14, 0.363]}>
        <planeGeometry args={[0.44, 0.008]} />
        <meshBasicMaterial color="#6BA9B5" />
      </mesh>
...
/* Station 5 · Calendar desk — big wall calendar + a few stationery items */
function CalendarDesk() {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        arr.push({
          x: (c - (cols - 1) / 2) * cellW,
          y: ((rows - 1) / 2 - r) * cellH,
          highlighted: r * cols + c === highlightI,
        });
      }
    }
    return arr;
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      {/* Calendar tile on back wall */}
      <group position={[0, 2.25, -0.34]}>
        <RoundedBox args={[1.56, 1.18, 0.05]} radius={0.035} smoothness={4} castShadow>
          <meshStandardMaterial color="#FDF6E2" roughness={0.9} />
        </RoundedBox>
        {/* Header strip */}
        <mesh position={[0, 0.48, 0.03]}>
          <planeGeometry args={[1.44, 0.16]} />
          <meshStandardMaterial color={station.accent} roughness={0.75} />
        </mesh>
        {/* Cells */}
        {cells.map((c, i) => (
          <mesh key={i} position={[c.x, c.y - 0.05, 0.028]}>
            <planeGeometry args={[0.18, 0.18]} />
            <meshBasicMaterial color={c.highlighted ? station.accent : "#EFE3C6"} />
          </mesh>
        ))}
        {/* Frame */}
        <RoundedBox args={[1.62, 1.24, 0.018]} radius={0.04} smoothness={4} position={[0, 0, -0.026]}>
          <meshStandardMaterial color="#6E5638" roughness={0.6} />
        </RoundedBox>
      </group>
      ...
      <group position={[0.55, 0.9, 0.2]}>
        ...
        {[-0.015, 0.02, -0.03].map((dx, i) => (
          <mesh key={i} position={[dx, 0.12, 0]} castShadow>
            <cylinderGeometry args={[0.006, 0.006, 0.22, 8]} />
            <meshStandardMaterial color={i === 0 ? "#C7543D" : i === 1 ? "#3C7A69" : "#54607A"} roughness={0.7} />
          </mesh>
        ))}
      </group>
