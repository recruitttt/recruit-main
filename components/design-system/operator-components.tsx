...
export function RunStatusIndicator({
  ...
  return (
    <div className={cx("flex items-center justify-between border border-[var(--glass-border)] bg-[var(--theme-compat-bg-soft)] px-3 py-2.5", mistRadii.nested)}>
      <div className="flex items-center gap-3">
        ...
        <span>
          ...
          {meta && <span className="block text-xs text-[var(--color-fg-subtle)]">{meta}</span>}
        </span>
      </div>
  ...
  return (
    <div className={cx("flex items-center justify-between px-3 py-3 text-sm", mistClasses.card)}>
      <div className="flex min-w-0 items-center gap-3">
        ...
        <span className="truncate font-semibold text-[var(--color-fg-muted)]">{title}</span>
      </div>
      <span className="shrink-0 text-[var(--color-fg-subtle)]">{meta}</span>
    </div>
  );
}
...
export function BrowserPreview({ children }: { children?: React.ReactNode }) {
  return (
    <div className={cx("border border-[var(--glass-border)] bg-[var(--theme-compat-bg-soft)] p-3", mistRadii.nested)}>
      <div className="mb-3 flex items-center gap-2 rounded-full border border-[var(--glass-border)] bg-[var(--theme-compat-bg-soft)] px-3 py-2">
        <span className="h-2 w-2 rounded-full bg-red-400" />
        <span className="h-2 w-2 rounded-full bg-amber-400" />
        <span className="h-2 w-2 rounded-full bg-emerald-400" />
        <span className="ml-2 truncate font-mono text-[10px] text-[var(--color-fg-subtle)]">greenhouse.io/linear/senior-product-manager</span>
      </div>
      <div className="min-h-52 rounded-[16px] border border-[var(--glass-border)] bg-[var(--theme-compat-bg-soft)] p-4">
        {children ?? <div className="flex h-44 items-center justify-center text-sm font-semibold text-[var(--color-fg-subtle)]">live browser preview</div>}
      </div>
    </div>
  );
}
